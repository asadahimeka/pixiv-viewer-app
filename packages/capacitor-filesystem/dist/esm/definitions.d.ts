...
export declare enum Directory {
     ...
     * On iOS it's not supported.
     * Use this directory to store user-generated content.
     * On Android it's the Public Downloads folder, so it's accessible from other apps.
     ...
     * On iOS it's not supported.
     * Use this directory to store user-generated content.
     * On Android it's the Public Pictures folder, so it's accessible from other apps.
     * It's not accesible on Android 10 unless the app enables legacy External Storage
     * by adding `android:requestLegacyExternalStorage="true"` in the `application` tag
     * in the `AndroidManifest.xml`.
     * On Android 11 or newer the app can only access the files/folders the app created.
...
export interface FilesystemPlugin {
    ...
    /**
     * Gets the size in bytes of the path.
     * If the path is a directory the size is the sum of the size of all the files in the
     * directory and sub-directories. If the path is a file, the size is the size of the file.
     *
     * @since 5.1.2-alpha.2
     */
    getFileSize(options: StatOptions): Promise<{
        size: number;
        len: number;
    }>;
    /**
